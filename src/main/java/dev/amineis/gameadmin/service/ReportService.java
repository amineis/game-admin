...
public class ReportService {
  ...
  @Transactional(readOnly = true)
  public PagedResponse<ReportResponse> getReportsByReportedPlayer(
      Long playerId, Pageable pageable) {
    return toPagedResponse(reportRepository.findByReportedPlayerId(playerId, pageable));
  }

  @Transactional(readOnly = true)
  public PagedResponse<ReportResponse> getReportsByReporterPlayer(
      Long playerId, Pageable pageable) {
  ...
  public ReportResponse createReport(ReportCreateRequest request) {
    ...
    Player reportedPlayer =
        playerRepository
            .findById(request.getReportedPlayerId())
            .orElseThrow(
                () -> new ResourceNotFoundException("Player", request.getReportedPlayerId()));
    Player reporterPlayer =
        playerRepository
            .findById(request.getReporterPlayerId())
            .orElseThrow(
                () -> new ResourceNotFoundException("Player", request.getReporterPlayerId()));
