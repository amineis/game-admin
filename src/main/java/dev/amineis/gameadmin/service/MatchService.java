...
public class MatchService {
    ...
    public MatchResponse getMatchById(Long id) {
        GameMatch match = gameMatchRepository.findById(id)
    ...
    public void removePlayerFromMatch(Long matchId, Long playerId) {
        ...
        if (!matchPlayerRepository.existsByMatchIdAndPlayerId(matchId, playerId)) {
            throw new ResourceNotFoundException("MatchPlayer not found for matchId " + matchId + " and playerId " + playerId);
        }
    ...
    private MatchResponse toMatchResponse(GameMatch match) {
        ...
        List<MatchPlayerResponse> players = matchPlayerRepository.findByMatchId(match.getId())
                .stream()
                .map(matchPlayerMapper::toResponse)
                .toList();
